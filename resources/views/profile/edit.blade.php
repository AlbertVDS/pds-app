...
@section('content')
                ...
                <table class="table table-bordered">
                    <tbody>
                        <tr>
                            ...
                            <td id="token" width="70%">

                            </td>
                            <td>
                                <a href="#" id="getToken" class="btn btn-primary">
    ...
    <script>
        $('#getToken').on('click', function () {
            $.ajax({
                url: {{ route('tokens.create', $user) }},
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
                    // If needed, include CSRF token:
                ...
                success: function (response) {
                    ...
                    if (response.token) {
                        ...
                        $('#token').text('No token returned');
                    }
                },
                error: function (xhr) {
                    $('#token').text('Error: ' + xhr.status);
                }
            });
        });
    </script>
