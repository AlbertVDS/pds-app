...
@section('content')
        ...
        <div class="mb-3">
            <label for="mailing-group" class="form-label">Mailing group</label>
            <select class="form-select" aria-label="Select mailing group" id="mailing-group" name="mailing_group_id">
                <option selected>Select mailing group</option>
                @foreach ($mailingGroups as $group)
                    <option value="{{ $group->id }}" {{ old('mailing_group_id', $mailing->mailing_group_id ?? '') == $group->id ? 'selected' : null }} >{{ $group->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            <label for="schedule-date" class="form-label">Scheduled for</label>
